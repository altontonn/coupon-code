    ...
    <div>
      ...
      <ul className="divide-y flex flex-col jusify-center items-center">
        {codes && codes.length > 0 ? (
          codes.map((code) => (
            <li className="py-4 flex items-center justify-between" key={code.id}>
              <span className="flex-grow">{code.passcode}</span>
              <button className={`bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded ml-6 ${isRedeemed(code.id) ? 'opacity-50 pointer-events-none' : ''}`} onClick={() => redeemCode(code.id)}>Redeem</button>
            </li>
          ))
        ) : (
